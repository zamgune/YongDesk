...
이 문서는 승인된 [초보자 중심 HTML v3 시안](ux-prototypes/macos-native/beginner-first-v3.html)을 실제 YongStockDesk SwiftUI 앱에 적용하기 위한 구현 기준이다. HTML을 그대로 복사하는 것이 아니라 현재 sidecar, 선택형 credential, 분석, 전략과 paper-only 안전 경계를 유지하면서 화면 구조를 교체한다.
...
이 문서의 구현이 끝날 때 [기능 상태](features.md)의 관련 항목을 `HTML 시안`·`명세만 존재`에서 `부분 구현` 또는 `구현됨`으로 갱신한다.
