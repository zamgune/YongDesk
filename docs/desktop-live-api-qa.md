...
이 문서는 로컬 결정론적 검증을 모두 통과한 YongStockDesk에 실제 Toss·Upbit 연결을 추가해 인수 확인하는 절차다. 1.1.0에서는 Toss 수동 지정가와 Upbit `KRW-*` 수동 지정가의 제한적 실 API 인수를 각각 제공하며, Bithumb과 코인 자동매매는 조회·paper 전용이다. 앱의 분석·paper 기능 준비와 외부 공개 배포 준비를 구분한다.
...
| Upbit 수동 실거래 인수 완료 | 로컬 기능 준비에 더해 Upbit 읽기 전용 QA, 수동 토글, `KRW-*` 지정가 1건의 typed confirmation·거래소 주문 ID 대조와 결과 불명 재조정 증거 완료 |
...
API 키 연결은 선택 사항이다. 다만 `dataSource=toss` 실 시세나 실계좌 잔고·주문 가능 정보까지 인수하려면 해당 절의 QA가 필요하다. Toss와 Upbit 실주문은 [1.1.0 운영 경계](live-trading-v1.1.md)의 QA와 별도 수동 토글을 완료한 경우에만 수행하며, Bithumb 주문은 지원하지 않는다.
...
- Toss·Upbit 수동 토글 OFF에서는 submit이 차단되고, 코인 자동화는 paper 기록만 남긴다.
...
2. `코인 실거래 QA 승인`을 입력한다. 다시 한 번의 `KRW-BTC` 읽기 전용 점검만 실행되고 주문 내역에는 새 항목이 없어야 한다.
3. `코인 실거래 수동 주문 해제`를 입력한다. 자동매매·시장가·Bithumb이 열리지 않는지 확인한다.
4. 최소 규모의 `KRW-*` 지정가를 사전검증한다. 화면에 표시된 주문 요약을 정확히 입력한 뒤에만 단 한 번 제출한다. `clientOrderId`, Upbit 주문 UUID, 거래소 주문 내역을 대조한다.
5. 429·5xx·timeout·응답 불명은 자동 재시도하지 않는다. 수동 토글이 잠긴 것을 확인하고 `결과 불명 주문 재조정`이 client order identifier 조회만 호출하는지 기록한다. 조회로 확정하지 못하면 토글을 계속 OFF로 둔다.
