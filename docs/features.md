...
| 배포 안전 | 1.2.0-beta.1 무실주문 잠금 | 구현됨 | Toss·Upbit·Bithumb 실제 제출·취소는 HTTP 423 `PRE_RELEASE_LIVE_LOCK`, 자동화는 paper-only | 내부 베타 전용 |
...
9. `내 자산`은 Toss·Upbit·Bithumb 실자산 포트폴리오 보드만 제공한다. 모의계좌 전환은 사용자 자산 화면에서 제거했지만, 1.2.0-beta.1의 자동화와 안전 검증을 위한 paper 엔진은 내부 경로로 유지한다. 실주문과 취소는 전역 잠금되며 모든 토글은 시작 시 OFF로 초기화된다.
