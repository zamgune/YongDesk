...
| 앱 셸 | 차트 중심 사이드바와 `차트 + 우측 인스펙터` 레이아웃 | 구현됨 | 1440×900 분할 화면, 최소 1024×720에서 접을 수 있는 인스펙터, 페이지 전체 스크롤 없음 | 없음 |
...
| 분석 | 1~3일 단기·스윙·장기 익절·손절 계획 | 구현됨 | 차트 인스펙터에서 진입·지지·저항·목표·무효화·손익비를 표시하고, 필수 봉·지표가 없으면 고정 퍼센트 대신 `계산 불가` 또는 `조건 대기` 반환 | 없음 |
...
| 분석 | 한국 주식 관심종목 장중 급락반등 감시 | 구현됨 | 검증된 Toss API·앱 실행·KRX 정규장 필요. 60초 REST polling으로 확정 5분봉과 직전 20거래일 동일 KST 구간 RVOL을 사용하고 최대 30세션을 로컬 누적하며, KOSPI 위험 배지·단계별 중복 방지 알림·additive `tradePlan`을 제공. 표본 누적 전에는 `unavailable` | 없음 |
...
| 민심 | macOS `민심 레이더`의 종목별 한국·해외 반응 비교 | 부분 구현 | `GET /api/local/sentiment-overview`가 가즈아·비관/비난·혼재·중립의 수집된 반응 비율과 FOMO·공포·공격성, 근거 링크를 제공. 기본 24시간이며 표본 5건 미만이면 72시간으로 확장 | 없음 |
| 민심 | Toss 거래대금 상위 30종목의 한국·미국 시장 비교 | 부분 구현 | `MARKET_TRADING_AMOUNT`, 1일 순위와 Reddit 연결이 모두 필요. 종목 동일가중 집계이며 Yahoo·고정 종목 fallback 없이 연결 조건이 부족하면 비교를 숨김 | 없음 |
...
| 자산 | 실제 보유 종목 장기 보유 관리 | 구현됨 | 평단·수익률·SMA200·주간/월간 무효화 기준을 읽기 전용으로 표시하고 `차트에서 확인`만 제공 | 없음 |
...
| 모의투자 | 관리형 매수와 익절·손절 자동 청산 | 구현됨 | 익절/손절 독립 토글 4조합, stale 시세 차단, 첫 청산 후 반대 leg 원자 취소, 재시작 복구 | 없음 |
...
| 주문 안전 | `ManagedTradePlan`과 통합 sidecar 계약 | 구현됨 | `/api/local/trade-plans/*`, 모든 진입·청산 leg에 개별 `OrderIntent`·`RiskCheck`, 분석값은 초안에만 복사 | 설치본 제출 잠금 |
...
| Toss | OpenAPI 1.2.4 조건주문 `SINGLE/OCO/OTO` | 구현됨 | 신규 매수+익절 또는 손절은 OTO, 보유분 1개 청산은 SINGLE, 보유분 익절+손절은 OCO. 신규 3단 브래킷은 차단 | 서명된 QA 패키지만 향후 가능 |
...
| 배포 안전 | 1.3.0-beta.1 실주문 잠금 | 구현됨 | 조건주문 코드는 포함하지만 일반·공개·임시 서명 빌드는 `liveSubmissionMode=disabled`; Toss·Upbit·Bithumb 제출·취소는 HTTP 423 | 내부 베타 전용 |
...
3. 왼쪽 기본 탐색은 `차트 → 섹터 → 민심 → 관심종목` 순서이며, 이어서 내 자산·전략·자동화와 설정에서 필요한 화면을 연다. 상단에서는 주식·코인과 종목을 선택한다.
...
6. 차트 화면은 최근 종가와 차트, 우측 `분석 / 주문 / 뉴스` 인스펙터를 동시에 표시한다. 분석에서는 1~3일 단기·스윙 계획과 신호·뉴스·민심 근거를 점진적으로 표시하고, 장기 계산은 실제 보유 종목의 자산 화면에서 사용한다.
7. 주문 인스펙터에서 `신규 매수 / 보유분 관리`, `모의 / Toss 실계좌`, 익절·손절 독립 토글을 설정한다. `분석값 채우기`는 가격만 복사하고 토글이나 제출을 대신 실행하지 않으며, 모의 주문은 사용자가 직접 진행해 실브로커를 호출하지 않는다.
8. 전략 설정은 `초안 저장 → 조건 확인 → 시뮬레이션 → 활성화` 순서와 scheduler·자동화 일시중지·kill switch 동작을 유지한다.
9. `내 자산`은 Toss·Upbit·Bithumb 실자산 포트폴리오와 선택한 실제 보유 종목의 장기 보유 점검을 제공한다. 1.3.0-beta.1 설치본은 조건주문 코드를 포함하지만 실제 제출과 취소를 전역 잠금한다.

문장형 전략 조립기는 여전히 HTML 시안이다. Beginner-first 레이아웃 적용이 기존 전략 계약이나 자동화 순서를 변경한 것은 아니다.

## 민심 레이더

- macOS 전용 `민심 레이더`는 `GET /api/local/sentiment-overview?symbol=<symbol>&market=KR|US`를 사용한다. `refresh=1`은 사용자가 현재 종목의 민심을 명시적으로 새로고침할 때만 붙이며, 자동 분석과 검색 결과 선택은 캐시를 사용한다. 기존 `/api/community-pain/:symbol`과 웹 `/sentiment`는 호환 경로로 유지한다.
- 각 한국·해외 버킷은 날짜가 확인된 최상위 글을 `bullish_hype`, `bearish_criticism`, `mixed`, `neutral` 중 하나로 분류한다. 네 정수 비율은 합계가 정확히 100이지만, 이는 고유 사용자나 여론조사 결과가 아니라 중복·작성자 상한을 적용한 **실험적인 수집된 반응 비율**이다.
- 기본 분모는 최근 24시간 글이다. 5건 미만이면 버킷 전체를 72시간으로 확장하고, 20건 이상은 `ready`, 5~19건은 `low_evidence`, 그 미만은 `unavailable`로 표시한다. 댓글·답글은 FOMO·공포와 근거에는 사용할 수 있지만 네 비율의 분모에는 넣지 않는다.
- 한국 종목의 한국 반응은 Paxnet, 한국 종목의 해외 반응과 미국 종목의 해외 반응은 Reddit OAuth를 사용한다. 미국 종목의 한국 반응은 현재 지원 소스가 없어 `unsupported_source_coverage`로 표시한다.
- 시장 비교는 Toss의 `MARKET_TRADING_AMOUNT`, 1일 기준 한국·미국 상위 30종목을 사용한다. 한국은 Paxnet, 미국은 Reddit 결과를 종목별 동일가중으로 집계한다. Toss 순위 또는 Reddit 인증이 없으면 Yahoo나 고정 유니버스로 대체하지 않고 시장 비교 전체를 숨긴다.
- 종목 성공 응답과 시장 집계의 TTL은 30분이다. 최초 시장 집계는 `warming`을 즉시 반환하고 백그라운드에서 한 번만 계산하며, App Support에는 원문이 아닌 집계 상태·비율·유니버스만 저장한다. 24시간 이내 이전 집계는 `stale`로 표시하면서 갱신할 수 있다.
- 민심 레이더는 참고용 데이터 화면이다. 결과는 매수 신호, 전략, `OrderIntent`, `RiskCheck`, broker submit 또는 주문 자동화 입력으로 연결하지 않는다. 상세 소스·캐시 정책은 [커뮤니티 반응 소스 정책](community-pain-sources.md)을 따른다.
...
- 손절·익절은 분석 참고값이며 자동 제출하지 않는다. 사용자가 주문 인스펙터에 복사한 뒤 토글·사전검증·제출을 각각 명시적으로 수행해야 한다.
