...
| 자산 | Toss·Upbit·Bithumb 통합 실자산 | 구현됨 | 공급자·계좌·통화별 표시, 15초 메모리 캐시, 30초 UI 갱신, stale/부분 실패 유지. 통화 간 합산 안 함 | 실 API 인수 필요 |
...
9. 실자산과 모의계좌는 `내 자산`에서 분리한다. Toss·Upbit·Bithumb 실거래는 자동 readiness·이용 동의·거래소별 토글을 통과한 단일 Mac/검증 API에서만 가능하며 모든 토글은 기본 OFF다.
