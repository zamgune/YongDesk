...
| 분석 | 단타·스윙 익절·손절 계획 | 구현됨 | 차트 인스펙터에서 진입·지지·저항·목표·무효화·손익비를 표시하고, 필수 데이터가 없으면 `계산 불가` 또는 `조건 대기` 반환 | 없음 |
...
6. 차트 화면은 차트와 우측 `분석 / 주문 / 뉴스` 인스펙터를 동시에 표시한다. 분석에서는 단타·스윙만 선택하며 장기 계산은 실제 보유 종목의 자산 화면에서 사용한다.
7. 주문 인스펙터에서 `신규 매수 / 보유분 관리`, `모의 / Toss 실계좌`, 익절·손절 독립 토글을 설정한다. `분석값 채우기`는 가격만 복사하고 토글이나 제출을 대신 실행하지 않는다.
