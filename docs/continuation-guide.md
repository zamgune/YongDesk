...
- `src/lib/trading/managed-trade-plan-store.ts`: 관리형 모의·조건주문 계획, 미리보기와 자동 청산 복구 저장소
- `src/adapters/toss/toss-conditional-order.ts`: Toss OpenAPI 1.2.4 `SINGLE/OCO/OTO` payload 변환
...
1.3.0-beta.1은 조건주문 코드 포함과 현재 설치본의 제출 가능 상태를 분리한다. 일반·공개·임시 서명 빌드는 환경변수와 저장된 동의보다 우선하는 `liveSubmissionMode=disabled` 고정 경계를 두며, Toss·Upbit·Bithumb 실제 submit·cancel을 HTTP 423 `PRE_RELEASE_LIVE_LOCK`으로 차단한다. 향후 `local-qa`는 서명된 QA 번들 capability로만 열고 환경변수만으로 전환하지 않는다.

## 거래 안전 경계

신호, 차트와 UI는 브로커를 직접 호출하지 않는다. Toss·Upbit·Bithumb 실제 제출에서 아래 순서는 생략할 수 없는 계약이다.
...
- `ManagedTradePlan`은 신규 매수·보유분 관리와 선택형 익절·손절을 묶는 상위 계약이다. 각 leg는 별도 `OrderIntent`·`RiskCheck`를 통과하고 10분 미리보기의 전체 입력 hash와 typed confirmation을 사용한다.
- 분석값 복사는 주문 초안만 변경하며 토글, 사전검증 또는 제출을 자동 수행하지 않는다. 차트와 급락 반전 신호도 broker를 직접 호출하지 않는다.

- Toss 등록 성공은 실거래 활성화가 아니다. 현재 Mac·선택 계좌의 자동 readiness, 이용 동의와 별도 수동 토글까지 완료해야 KR/US 지정가 주문을 제출할 수 있다.
- Toss 자동화는 수동 지정가 5건, 재시작 후 재조정, 결과 불명 0건, kill switch·worker pause 증거 뒤에만 별도 토글로 열 수 있다.
- Upbit·Bithumb 등록 성공은 실거래 활성화가 아니다. 자동 readiness, 이용 동의, 거래소별 토글, `KRW-*` 지정가 사전검증과 주문 요약 재입력이 필요하다.
- 제출 직전에는 `OrderIntent`·`RiskCheck`, 잔고·수수료·최소 주문금액·호가 단위·현재가 신선도를 다시 검증한다. 429·5xx·응답 불명은 자동 재시도하지 않고 Upbit `identifier` 또는 Bithumb `client_order_id` 조회 전까지 해당 거래소를 잠근다.
- 코인 자동화는 거래소별 수동 주문 5건, 재시작 재조정, kill switch 증거, 결과 불명 0건 뒤 별도 토글로 열며 체결 조회가 완료된 수량만 전략 단계에 반영한다.
