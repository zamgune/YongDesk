...
`ENABLE_LIVE_TRADING`과 `ENABLE_CRYPTO_LIVE_TRADING`은 웹·서버 호환 경계로 남아 있다. macOS 1.1.0 Toss 실거래는 이 환경변수로 열지 않으며, 로컬 정책 원장의 설치·계좌 QA, 수동/자동화 토글을 sidecar가 fail-closed로 검사한다. Upbit는 별도 로컬 정책 원장에서 읽기 전용 QA·수동 토글·주문 요약 재입력을 통과한 KRW 수동 지정가만 허용하며, Bithumb과 코인 자동매매는 계속 차단한다.

## 거래 안전 경계

신호, 차트와 UI는 브로커를 직접 호출하지 않는다. Toss 실제 제출과 Upbit 수동 지정가 제출에서 아래 순서는 생략할 수 없는 계약이다. 코인 자동매매와 Bithumb broker submit은 계속 이 경계 전에 차단한다.
...
- Toss 등록 성공은 실거래 활성화가 아니다. 1.1.0에서는 현재 Mac·선택 계좌의 읽기 전용 QA 승인과 별도 수동 토글까지 완료해야 KR/US 지정가 주문을 제출할 수 있다.
- Toss 자동화는 수동 지정가 5건, 재시작 후 재조정, 결과 불명 0건, kill switch·worker pause 증거 뒤에만 별도 토글로 열 수 있다.
- Upbit 등록 성공은 실거래 활성화가 아니다. 읽기 전용 QA 승인, `코인 실거래 수동 주문 해제`, `KRW-*` 지정가 사전검증과 주문 요약 재입력을 모두 완료해야 해당 Mac의 수동 submit 경로에 도달한다.
- Upbit 제출 직전에는 `OrderIntent`·`RiskCheck`, 잔고·수수료·최소 주문금액·호가 단위·현재가 신선도를 다시 검증한다. 429·5xx·응답 불명은 자동 재시도하지 않고 식별자 조회 전까지 수동 토글을 잠근다.
- Bithumb과 모든 코인 자동매매는 paper-only다.
