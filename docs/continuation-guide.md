...
1.2.0-beta.2는 환경변수와 저장된 동의보다 우선하는 `liveSubmissionMode=disabled` 고정 경계를 둔다. Toss·Upbit·Bithumb 실제 submit·cancel은 HTTP 423 `PRE_RELEASE_LIVE_LOCK`으로 차단되고, sidecar 시작 시 수동·자동 토글을 OFF로 초기화하며 자동화 cycle은 paper-only다.
