...
`ENABLE_LIVE_TRADING`과 `ENABLE_CRYPTO_LIVE_TRADING`은 웹·서버 호환 경계로 남아 있다. macOS 1.1.0 Toss 실거래는 이 환경변수로 열지 않으며, 로컬 정책 원장의 설치·계좌 QA, 수동/자동화 토글을 sidecar가 fail-closed로 검사한다. 코인 live 제출은 계속 차단한다.

## 거래 안전 경계

신호, 차트와 UI는 브로커를 직접 호출하지 않는다. Toss 실제 제출에서 아래 순서는 생략할 수 없는 계약이며 코인 broker submit은 계속 이 경계 전에 차단한다.
