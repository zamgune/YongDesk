...
`ENABLE_LIVE_TRADING`과 `ENABLE_CRYPTO_LIVE_TRADING`은 상위 fail-closed 경계다. macOS 로컬 정책 원장은 Toss·Upbit·Bithumb마다 자동 읽기 전용 readiness, credential/계좌 binding hash, 이용 동의, 수동·자동 토글을 추가로 검사한다. API 또는 Toss 거래 계좌가 바뀌면 readiness와 토글은 즉시 초기화된다.
