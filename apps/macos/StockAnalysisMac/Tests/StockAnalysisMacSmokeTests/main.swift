...
func verifyInteractiveChartWorkbenchSource() throws {
    ...
    assert(chartView.contains("rsi.createPriceLine"), "RSI guide lines must be created from the RSI series")
    assert(!chartView.contains("rsiChart.addPriceLine"), "chart API must not create price lines from the chart object")
    assert(chartView.contains("unhandledrejection"), "chart runtime errors should be surfaced to SwiftUI")
