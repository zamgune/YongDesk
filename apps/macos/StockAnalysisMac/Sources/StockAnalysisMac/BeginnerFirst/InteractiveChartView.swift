...
struct InteractiveChartOptions {
    ...
    let showMA60: Bool
    let showRSI: Bool
    let entryGuide: Double?
    let takeProfitGuide: Double?
    let stopLossGuide: Double?
    let resetToken: Int
    let reloadToken: Int
}
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        private func chartPayload(analysis: MarketAnalysisSnapshot?, options: InteractiveChartOptions) -> String {
            ...
            let guides: [[String: Any]] = [
                ["title": "진입", "price": options.entryGuide.map { $0 as Any } ?? NSNull(), "color": "#44c6e8"],
                ["title": "익절", "price": options.takeProfitGuide.map { $0 as Any } ?? NSNull(), "color": "#38d9a9"],
                ["title": "손절", "price": options.stopLossGuide.map { $0 as Any } ?? NSNull(), "color": "#ff6b7a"],
            ]
...
private enum InteractiveChartPage {
    ...
    <script>
    (()=>{const post=(type,payload={})=>window.webkit?.messageHandlers?.chartStatus?.postMessage({type,...payload});const fail=error=>post('error',{message:error instanceof Error?(error.stack||error.message):String(error||'차트 오류')});let priceChart,rsiChart,candles,volume,ma5,ma20,ma60,rsi,markers,guideLines=[],lastReset=-1,currentSignals=new Map();
