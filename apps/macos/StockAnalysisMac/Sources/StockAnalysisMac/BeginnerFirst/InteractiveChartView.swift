...
private enum InteractiveChartPage {
    ...
    <script>
    (()=>{const post=(type,payload={})=>window.webkit?.messageHandlers?.chartStatus?.postMessage({type,...payload});let priceChart,rsiChart,candles,volume,ma5,ma20,ma60,rsi,markers,lastReset=-1,currentSignals=new Map();
    ...
    document.addEventListener('DOMContentLoaded',init);window.addEventListener('error',event=>post('error',{message:event.message||'차트 오류'}));})();
    </script></body></html>
    """#
}
