...
const calculateDayPlan = (context: HorizonPlanContext): HorizonExitPlan => {
  ...
  if (!values) {
    ...
    if (values.higherTimeframeTrendUp === null) {
      missing.push(values.higherTimeframe === "4h" ? "4시간봉 추세 필터" : "일봉 위험 필터");
    }
    if (values.entryTrendUp === null) missing.push("1시간봉 진입 추세");
    if (values.trendQualityPassed === null) missing.push("1시간봉 추세 품질");
    if (values.volumeConfirmed === null) missing.push("1시간봉 거래량 확인");
  }
...
const calculateSwingPlan = (context: HorizonPlanContext): HorizonExitPlan => {
  ...
  if (!values) {
    ...
    if (values.marketGatePassed === null) missing.push("종목 일봉 위험 게이트");
    if (values.dailyTrendUp === null) missing.push("일봉 추세");
    if (values.entryTrendUp === null) missing.push(`${values.entryTimeframe === "4h" ? "4시간봉" : "1시간봉"} 진입 추세`);
    if (values.confirmationTimeframe && values.confirmationTrendUp === null) {
      missing.push(`${values.confirmationTimeframe === "1h" ? "1시간봉" : values.confirmationTimeframe} 재확인`);
    }
  }
  if (
    missing.length ||
...
const calculateLongPlan = (context: HorizonPlanContext): HorizonExitPlan => {
  ...
  if (!values) {
    ...
    if (!finitePositive(values.weeklySma60)) missing.push("주봉 SMA60");
    if (values.marketGatePassed === null) missing.push("장기 종목 일봉 위험 게이트");
  }
  if (
    missing.length ||
    !values ||
    ...
    !finitePositive(values.weeklySma60)
  ) {
    return unavailablePlan({
      context,
      horizon: "long",
  ...
  const blockers = [
    ...
    values.weeklySma20 <= values.weeklySma60 ? "주봉 SMA20이 SMA60 위에 있지 않습니다." : null,
    values.weeklySma20 >= context.entryPrice ? "현재가가 주봉 SMA20 추적선 위에 있지 않습니다." : null,
  ].filter((value): value is string => value !== null);
